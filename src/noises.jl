...
function optquantiz(scenarios, nbins; scale=true)
    ...
    for t in 1:ntime
        ...
        push!(output, NoiseLaw(center, 1/nscenarios*quantiz.counts))
    end
    return output
end
...
function fitgloballaw(names, nstages, nbins; nscen=10)
    ...
    for (idx, name) in enumerate(names)
        # inflow (in GWh)
        inflow = 24*readdlm("data/scen/$name/inflow.txt", ',')[1:nscen, 1:ntime]
        # demand (in GWh)
        demand = 24*readdlm("data/scen/$name/demands.txt", ',')[1:nscen, 1:ntime]
        nscen = size(inflow, 1)
        ...
        for nn in 1:nstages
            s1 = nn
            s2 = nn + 1
            _inflow[:, nn] = sum(inflow[:, s1:s2], dims=2)
            _demand[:, nn] = sum(demand[:, s1:s2], dims=2)
        end
